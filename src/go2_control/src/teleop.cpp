...
private:
    bool is_lock_ = true;  // Variable pour savoir si le robot est verrouillé
    ...
    void input_callback(const sensor_msgs::msg::Joy::SharedPtr msg)
    {
        ...
        float right_stick_x = msg->axes[3]; // Axe horizontal du stick droit
        float right_stick_y = msg->axes[4]; // Axe vertical du stick droit
        float dpad_vertical = msg->axes[7];  // Axe vertical de la croix directionnelle (haut/bas)
        ...
        x_pressed_ = (msg->buttons[3] == 1);
        a_pressed_ = (msg->buttons[0] == 1);
        b_pressed_ = (msg->buttons[1] == 1);
        y_pressed_ = (msg->buttons[2] == 1);
